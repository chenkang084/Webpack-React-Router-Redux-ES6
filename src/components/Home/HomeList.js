...
const returnTop = con => {
  if (localItem("scrollPosition")) {
    ...
    try {
      con.refs.lv.refs.listview.refs.listviewscroll.refs.ScrollView.scrollTop = localItem(
        "scrollPosition"
      );
    } catch (e) {
      console.log(e);
...
class HomeList extends React.Component {
  ...
  componentWillUnmount() {
    localItem(
      "scrollPosition",
      this.refs.lv.refs.listview.scrollProperties.offset
    );
  }
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      this.props.homeListObj.homeDataList.length !=
        nextProps.homeListObj.homeDataList.length ||
      this.props.homeListObj.articleType != nextProps.homeListObj.articleType ||
      this.props.homeListObj.pageIndex != nextProps.homeListObj.pageIndex
    );
  }
          ...
          <div className="titleArea" data-flex="dir:top main:left cross:top">
            <h4 className="itemTitle">
              ...
              >
                {rowData.top
                  ? "置顶"
                  : rowData.good ? "精华" : articleType[rowData.tab]}
              </span>
            </h4>
            <p className="someInfo">
              <span className="infoSpan">查看：{rowData.visit_count}</span>
              <span className="infoSpan">回复：{rowData.reply_count}</span>
              <span className="infoSpan">
                最后回复：{formatDate(rowData.last_reply_at)}
              </span>
            </p>
          </div>
        </div>
      ...
      <div style={{ margin: "0 auto", width: "100%" }}>
        <ListView
          ...
          renderHeader={() => (
            ...
            >
              {articleType[homeListObj.articleType]}
            </div>
          )}
          renderFooter={() => (
